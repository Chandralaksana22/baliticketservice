<template>
    <RouterLink to="/ticket-detail" class="grid grid-cols-3 gap-3 pt-5 items-start" style="border-bottom: 1px solid #9ca3af;">
        <div class="col-span-1">
            <img :src="imageSrc" :alt="imageAlt" class="rounded-2xl h-[130px] object-cover" />
        </div>
        <div class="col-span-2">
            <button class="bg-[#926000] rounded-lg p-2 text-white text-xs uppercase">{{ buttonText }}</button>
            <div class="grid grid-cols-2 mt-2 gap-1">
                <button class="bg-[#FFD600] rounded-lg p-2 text-black text-[10px] font-bold">{{ sellingTicketText
                    }}</button>
            ...
            <div class="my-5">
                ...
                <p class="text-md text-[#EF681C] font-bold">
                    {{ newPrice }} <span class="text-sm text-black">/person</span>
                </p>
            </div>
        </div>
    </RouterLink>
